...
export default function TaxBenefit({
  eyebrow = 'Rationaler Hebel',
  headline = 'Steuervorteil nutzen –',
  headlineHighlight = 'für alle Seiten',
  body = 'Essenszuschüsse sind steuerlich begünstigt. Das bedeutet: Ihr Unternehmen spart Lohnnebenkosten und Ihre Mitarbeitenden bekommen mehr netto vom Brutto. Eine echte Win-win-Situation.',
  highlights = FALLBACK_HIGHLIGHTS,
  figures = FALLBACK_FIGURES,
  disclaimer = '* Richtwerte gem. §8 Abs. 2 EStG. Individuelle steuerliche Beratung empfohlen.',
}) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <ScrollReveal direction="left">
            ...
            <h2 className="font-headline font-extrabold text-4xl md:text-5xl text-brand-text leading-tight mb-6 text-balance">
              {headline}<br />
              <span className="text-brand-green-light">{headlineHighlight}</span>
            </h2>
            ...
            <ul className="flex flex-col gap-4">
              {highlights.map((item) => (
                <li key={item} className="flex items-start gap-3 font-body text-base text-brand-text">
                  <CheckCircle2 size={20} className="text-brand-green shrink-0 mt-0.5" strokeWidth={2} aria-hidden="true" />
                  {item}
                </li>
              ))}
            </ul>
            ...
            <div className="relative overflow-visible">
              ...
              <div className="bg-white rounded-4xl p-10 max-md:px-6 shadow-card-lg relative z-10">
                <div>
                  ...
                  <p className="mt-6 font-body text-xs text-brand-text">{disclaimer}</p>
                </div>
              </div>
            </div>
          </ScrollReveal>
